...
namespace NSWeb.Controllers
{
    public class QAController : Controller
    {
        ...
        {
            //ViewData["files"] = DBContext.QAUploadInfo.Where(x => x.IsDel == 0).OrderByDescending(x => x.CreateTime).ToList();
            return View("Main");
        }
        [ActionName("fileList")]
        public ContentResult GetFileList()
        {
            var infos = DBContext.QAUploadInfo.Where(x => x.IsDel == 0).OrderByDescending(x => x.CreateTime).ToList();
                ...
                if (files != null && files.Count > 0)
                {
                    foreach (string file in Request.Files)
                    {
                        ...
                        if (item != null && item.ContentLength > 0)
                        {
                            ...
                            DBContext.QAUploadInfo.Add(new DataService.QAUploadInfo()
                                {
        ...
        {
            var info = DBContext.QAUploadInfo.FirstOrDefault(x => x.Id == id);
            if (info == null)
            {
        ...
        public FileStreamResult DownFileTrim(int id)
        {
            ...
            var tcFile = DBContext.QAUploadInfo.FirstOrDefault(x => x.Name == tcName);
            if (tcFile != null)
            {
        ...
        public FileStreamResult DownAnaysleFile(int id)
        {
            ...
            string file = service.CreateQA(absoluFilePath);
        ...
        private Dictionary<string, object> GetAssistParams(int id)
        {
            ...
            var currentInfo = DBContext.QAUploadInfo.FirstOrDefault(x => x.Id == id);
            if (currentInfo != null)
            {
               // list.Add("CurrentInfo", currentInfo);
                string trimFileName = Path.GetFileNameWithoutExtension(currentInfo.Name) + "_tc.xls";
                var trimInfo = DBContext.QAUploadInfo.FirstOrDefault(x => x.Name == trimFileName && x.IsDel.Value==0);
                ...
                var mydInfo = DBContext.QAUploadInfo.FirstOrDefault(x => x.Name == mydFileName && x.IsDel.Value == 0);
                ...
                var tslInfo = DBContext.QAUploadInfo.FirstOrDefault(x => x.Name == tslFileName && x.IsDel.Value == 0);
